...
export default function Boards() {
  ...
  const openModal = (board) => setSelectedBoard(board);
  const closeModal = () => setSelectedBoard(null);

  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      {/* Header */}
      <div className="text-center mt-16 px-4">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Boards Collection</h1>
        <p className="text-gray-600 mt-2 text-sm md:text-base">Explore our variety of boards suitable for furniture, construction, and interior design.</p>
      </div>
      ...
      {selectedBoard && (
        <div className="fixed inset-0 bg-black bg-opacity-70 p-4 flex items-center justify-center z-50" onClick={closeModal}>
          <div className="bg-white rounded-lg overflow-hidden max-w-lg w-full relative" onClick={(e) => e.stopPropagation()}>
            ...
            <img src={selectedBoard.images[0]} alt={selectedBoard.title} className="w-full h-[600px] object-contain" />
          </div>
        </div>
      )}
    </div>
