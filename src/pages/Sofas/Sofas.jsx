...
export default function Sofas() {
  ...
  const openModal = (sofa) => setSelectedSofa(sofa);
  const closeModal = () => setSelectedSofa(null);

  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      <div className="text-center mt-16 px-4">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Sofas Collection</h1>
        <p className="text-gray-600 mt-2 text-sm md:text-base">Browse our stylish and modern sofas curated for your living room.</p>
      </div>
      ...
      {selectedSofa && (
        <div className="fixed inset-0 bg-black bg-opacity-70 p-4 flex items-center justify-center z-50" onClick={closeModal}>
          <div className="bg-white rounded-lg overflow-hidden max-w-lg w-full relative" onClick={(e) => e.stopPropagation()}>
            ...
            <img src={selectedSofa.images[0]} alt={selectedSofa.title} className="w-full h-[600px] object-contain" />
          </div>
        </div>
      )}
    </div>
