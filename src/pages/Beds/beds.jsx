...
export default function Beds() {
  ...
  const openModal = (bed) => setSelectedBed(bed);
  const closeModal = () => setSelectedBed(null);

  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      <div className="text-center mt-16 px-4">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800">Beds Collection</h1>
        <p className="text-gray-600 mt-2 text-sm md:text-base">Browse our stylish and comfortable beds for every bedroom.</p>
      </div>
      ...
      {selectedBed && (
        <div className="fixed inset-0 bg-black bg-opacity-70 p-4 flex items-center justify-center z-50" onClick={closeModal}>
          <div className="bg-white rounded-lg overflow-hidden max-w-lg w-full relative" onClick={(e) => e.stopPropagation()}>
            ...
            <img src={selectedBed.images[0]} alt={selectedBed.title} className="w-full h-[600px] object-contain" />
          </div>
        </div>
      )}
    </div>
